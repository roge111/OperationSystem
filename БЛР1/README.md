...
`Shell.c` — реализация командной оболочки с поддержкой логического оператора `&&`.

Содержит следующие функции:

- `execute_command` — выполняет команду с поддержкой логического оператора `&&`;
- `run_single_command` — выполняет одну команду через `vfork()`;
- `split_commands` — разделяет команды по оператору `&&`;
- `parse_args` — разбивает строку на аргументы;
- `execute_builtin` — выполняет встроенные команды (`cd`, `pwd`, `ls`, `echo`, `short-path`, `cls`, `help`);
- Вспомогательные функции для каждой встроенной команды.
...
1) Оболочка запускается с приглашением ввода `$`.
2) Пользователь вводит команду, которая обрабатывается функцией `execute_command`.
3) При вводе `exit` оболочка завершает работу.

##### execute_command

1) Удаляет пробелы в начале и конце команды.
2) Разделяет команды по оператору `&&` с помощью функции `split_commands`.
3) Для каждой команды:
   - Удаляет пробелы вокруг команды
   - Парсит аргументы с помощью `parse_args`
   - Выполняет команду через `run_single_command`
   - Если команда завершилась с ошибкой и есть следующая команда, выполнение прекращается
4) Освобождает память, выделенную для хранения команд.

##### split_commands

1) Разбивает строку команд на отдельные команды по оператору `&&`.
2) Выделяет память для каждой команды и копирует в нее строку.
3) Возвращает массив команд и их количество.

##### run_single_command

1) Создает дочерний процесс с помощью `vfork()`.
2) В дочернем процессе:
   - Проверяет, является ли команда встроенной с помощью `execute_builtin`
   - Если встроенная - выполняет и завершает процесс
   - Если внешняя - запускает через `execvp`
3) В родительском процессе ждет завершения дочернего и возвращает статус выполнения.

##### Встроенные команды

Реализованы следующие встроенные команды:
- `cd [dir]` — смена текущей директории
- `pwd` — вывод текущей директории
- `ls [dir]` — вывод содержимого директории (без скрытых файлов)
- `echo [text]` — вывод текста с поддержкой перенаправления `>` и `>>`
- `short-path` — вывод полного пути к файлу
- `cls` — очистка экрана
- `help` — вывод справки по командам

### Итог. Часть 1. Shell.c
Реализована командная оболочка с поддержкой логического оператора `&&` и встроенными командами.
...
Нагрузчик на память — это программа, которая нагружает дисковую подсистему за счет случайного чтения блоков данных из файла с отключенным кэшированием и выполнения операций записи. Для вычисления метрик программа использует уже готовые функции из `CpuLoader.c` — `process_monitor_thread` и `cpu_monitoring`. Суть в том, что нет смысла вновь реализовывать их, так как они уже реализованы в `CpuLoader.c`. Соответственно, программа использует и те же структуры данных, что и в `CpuLoader.c`.

Программа-нагрузчик расположена в файле `MemoryLoader.c`.

Реализует следующие функции:

- `randomReadTest` — функция, которая выполняет случайное чтение блока данных из файла с отключенным кэшированием, поиск и замену числа в блоке, а также запись измененного блока в файл. Также выполняет дополнительную нагрузку на диск через создание и запись временных файлов с флагом `O_SYNC`.
...
- `use_data` — вспомогательная функция для предотвращения оптимизации компилятором.

Особенности реализации:

- Используется флаг `O_DIRECT` при открытии файла для отключения кэширования на уровне операционной системы.
- Вызывается `posix_fadvise` с флагом `POSIX_FADV_DONTNEED` для указания ядру, что данные больше не понадобятся.
- Чтение выполняется блоками размером 1 МБ по случайным смещениям в файле.
- После чтения выполняется поиск и замена указанного числа в прочитанном блоке.
- Выполняется дополнительная нагрузка на диск через создание 10 временных файлов и многократную запись в них с флагом `O_SYNC`.

Во время выполнения операций чтения и записи в параллельных потоках замеряются метрики CPU и количество процессов, как и в случае с нагрузчиком на CPU.

Результаты замеров записываются в указатели, переданные в функцию.
...
Определить примерное время чтения/записи файла сложно, но можно предположить, что время чтения/записи файла будет пропорционально количеству нагрузчиков. Также ожидается рост метрики `%wait` с увеличением числа нагрузчиков.

Гипотеза: примерное время зависит от многих факторов, включая железо, ожидается рост числа переключений контекста с увеличением числа нагрузчиков, а `wait` останется почти неизменным из-за невозможности одновременного запуска нескольких потоков. Ожидается рост `%user` и времени работы с увеличением числа нагрузчиков. Подробности об этих метриках можно почитать в разделе анализа нагрузки на CPU. `%sys` уменьшися, так как система будет тратить больше времени на IO, нежели на системные вызовы. 
...
![alt text](images/image-memory_wait.png)


Исходя из графика, видно, что `%wait` увеличивается с увеличением числа нагрузчиков. Что и предполагалось в гипотезе. Это происходит из-за того, что `%wait` - время затраченное на IO, а с число нагрузчиков на память нагрузка на IO тоже растет. 

#### Количество переключений контекста

Количество переключений контекст остается незменным, что может говорить о том, что процесс выполняется на одном ядре. Так же в ОС просто может не быть в момент измерения других активных процессов

#### Время работы, %USER, %SYS

Что время работы, что `%user` увеличиваются с увеличением числа нагрузчиков.

#### %SYS

![alt text](images/image-memory_sys.png)

`%sys` уменьшается с увеличением числа нагрузчиков.
...
steps = [
                ...
                (20, 2),  # умеренная память при пороге CPU
                (20, 4),  # равные нагрузки на CPU и память
                (30, 2),  # CPU выше порога, низкая память
                (30, 5),  # средний уровень памяти при повышенном CPU
                (40, 5),  # высокий CPU, средняя память
                (40, 7),  # почти максимальная память при высоком CPU
                (50, 3),  # пиковый CPU, умеренная память
                (50, 7),  # высокая нагрузка на оба ресурса
                (50, 12)  # максимальная нагрузка на память и CPU
        ]
...
В ходе лабораторной работы была реализована командная оболочка с поддержкой логического оператора `&&` и встроенными командами. Также были разработаны два нагрузчика: на CPU (вычисление хеша MD5) и на память (чтение/запись файлов с отключенным кэшированием).

Было проведено исследование поведения ОС во время исполнения разработанных программ-нагрузчиков. Я разобрался в том, что такое `%user`, `%sys`, `%wait`, `Количество переключений контекста` и изучил зависимость этих метрик от нагрузки на память и CPU.

Особенно интересным оказалось наблюдать за тем, как справляется ОС и моя система с высокими нагрузками. Также для меня это стало прекрасной возможностью углубиться в изучение Linux.
...
Перед операционной системой стояли две проблемы:

- Низкая скорость работы жестких жисков
- Совместное использование ранее загруженных файлов разными программами

Эти две пробоемы были решены одним  махом - page cahe или страничный кеш. Он используется ядром и файлы имеют единный размер - страница (обычно 4 кб). Page Cashe позволяет при первом использовании файла сохранить его копию в кеш, а при следюущих обращаниях файл берется не из жесткого диска, а уже из кеша - что гораздо ускоряет работы системы. И когда идет запись данных в диск, то тут тоже учавствует page cashe. Санчала идет запись в кеш, а потом уже на диск. 

В страничном кеше есть несколько типов страницы (своего рода статусы):

- Clean pages - идентичны данным на диске (то есть они могут быть безопасно удалены без потери данных, так как точно такие же уже есть на диске)
- Dirty pages - измененные данные, но не записаны на диск
- Inactive pages - редко используемы (кандидаты на вытиснение из кеша)
- Active pages - часто используются (это по сути метка приоритета. То есть такие файлы имеют высокий приоритет для удержания в кеше)
- Locked pages - заблокированные для I/O


Однако при каждом обычном чтении мы еще создаем копию в буфер из Page Cash. И так представьте много процессов. ДЛя решения такой проблемы стали использовать `прямое отображение` (mmap). Тут все изменения или чтение идет напрямую из Page Cash - без создания копии в буфер.



Есть два вида управления памятью - `MMU` и `TLB`. 

MMU - это система, которая по сути и проверяет наличие страницв страничном кеше и подгружать при необходимости. 

TLB - это система, которая позволяет увелчить размер страницы. То есть кеширует уже по 2 Мб вместо 4 Кб